defmodule HelpdeskWeb.HelpdeskLive.Admin.Index do
  ...
  def mount(_params, _session, socket) do
    case Operations.load(socket.assigns.current_user) do
      {:ok, report} ->
        {:ok,
         socket
  ...
  def handle_event("refresh", _, socket) do
    ...
    case Operations.load(actor) do
      {:ok, report} -> {:noreply, assign_report(socket, report)}
      {:error, :forbidden} -> {:noreply, redirect(socket, to: ~p"/tickets")}
    end
  end
